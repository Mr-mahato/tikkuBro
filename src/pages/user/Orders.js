...
const Orders = () => {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get("https://ecom-backend-gk0w.onrender.com/api/v1/auth/orders");
      setOrders(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCancel = async (orderId) => {
    try {
      const { data } = await axios.put(`https://ecom-backend-gk0w.onrender.com/api/v1/auth/cancel-order/${orderId}`);
